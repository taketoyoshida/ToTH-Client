...
private CompletableFuture<Boolean> sendCancelMatchingRequest(Socket s) {
	...
	private JPanel createOthelloPanel(int row, String myName, String color) {
        ...
        for(int i = 0 ; i < row ; i++){
            for(int j=0;j<row;j++) {
           	    if(board[i][j]==0){ buttonArray[i][j] = new JButton(boardIcon);}//盤面状態に応じたアイコンを設定
                if(board[i][j]==1){ buttonArray[i][j] = new JButton(blackIcon);}//盤面状態に応じたアイコンを設定
                if(board[i][j]==2){ buttonArray[i][j] = new JButton(whiteIcon);}//盤面状態に応じたアイコンを設定
                if(board[i][j]==-1){ buttonArray[i][j] = new JButton(redIcon);}//盤面状態に応じたアイコンを設定
    ...
    public void updateBoard() {
        ...
        for(int i = 0 ; i < row ; i++){
             for(int j=0;j<row;j++) {
            	 if(othello.board[i+1][j+1]==1){ buttonArray[i][j] = new JButton(blackIcon);}//盤面状態に応じたアイコンを設定
                 if(othello.board[i+1][j+1]==2){ buttonArray[i][j] = new JButton(whiteIcon);}//盤面状態に応じたアイコンを設定
